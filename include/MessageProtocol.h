 ...
 *
 * This system provides separate, dedicated enums for external vs internal
 * messages to ensure unmistakable separation and type safety:
 *
 * EXTERNAL MESSAGES: Cross transport boundaries (Serial in normal mode, network
 * in OTA mode)
...
inline bool isValidExternalMessageType(ExternalMessageType type) {
  return type != ExternalMessageType::INVALID &&
         static_cast<int16_t>(type) >= 0 && static_cast<int16_t>(type) <= 5;
}

inline bool isValidInternalMessageType(InternalMessageType type) {
  return type != InternalMessageType::INVALID &&
         type != InternalMessageType::UNKNOWN;
}
 ...
 * Implementation details are hidden to avoid compilation issues with STL
 * containers
 */
...
#define EXTERNAL_MSG_TYPE_TO_STRING(type)                                      \
  MessageProtocol::externalMessageTypeToString(type)
#define INTERNAL_MSG_TYPE_TO_STRING(type)                                      \
  MessageProtocol::internalMessageTypeToString(type)
#define STRING_TO_EXTERNAL_MSG_TYPE(str)                                       \
  MessageProtocol::stringToExternalMessageType(str)
#define STRING_TO_INTERNAL_MSG_TYPE(str)                                       \
  MessageProtocol::stringToInternalMessageType(str)
#define IS_VALID_EXTERNAL_MSG_TYPE(type)                                       \
  MessageProtocol::isValidExternalMessageType(type)
#define IS_VALID_INTERNAL_MSG_TYPE(type)                                       \
  MessageProtocol::isValidInternalMessageType(type)
#define EXTERNAL_MSG_PRIORITY(type)                                            \
  MessageProtocol::getExternalMessagePriority(type)
#define INTERNAL_MSG_PRIORITY(type)                                            \
  MessageProtocol::getInternalMessagePriority(type)
#define EXTERNAL_MSG_CATEGORY(type)                                            \
  MessageProtocol::getExternalMessageCategory(type)
#define INTERNAL_MSG_CATEGORY(type)                                            \
  MessageProtocol::getInternalMessageCategory(type)
...
#define CAST_TO_EXTERNAL_MSG_TYPE(intValue)                                    \
  static_cast<MessageProtocol::ExternalMessageType>(intValue)
#define CAST_TO_INTERNAL_MSG_TYPE(intValue)                                    \
  static_cast<MessageProtocol::InternalMessageType>(intValue)
...
#define SAFE_DESERIALIZE_EXTERNAL_MSG_TYPE(jsonDoc, field)                     \
  ([&]() {                                                                     \
    int typeNum =                                                              \
        jsonDoc[field] |                                                       \
        static_cast<int>(MessageProtocol::ExternalMessageType::INVALID);       \
    auto type = static_cast<MessageProtocol::ExternalMessageType>(typeNum);    \
    return MessageProtocol::isValidExternalMessageType(type)                   \
               ? type                                                          \
               : MessageProtocol::ExternalMessageType::INVALID;                \
  })()
...
#define SAFE_DESERIALIZE_INTERNAL_MSG_TYPE(jsonDoc, field)                     \
  ([&]() {                                                                     \
    int typeNum =                                                              \
        jsonDoc[field] |                                                       \
        static_cast<int>(MessageProtocol::InternalMessageType::INVALID);       \
    auto type = static_cast<MessageProtocol::InternalMessageType>(typeNum);    \
    return MessageProtocol::isValidInternalMessageType(type)                   \
               ? type                                                          \
               : MessageProtocol::InternalMessageType::INVALID;                \
  })()
...
namespace std {
template <> struct hash<MessageProtocol::ExternalMessageType> {
  size_t
  operator()(const MessageProtocol::ExternalMessageType &e) const noexcept {
    return hash<int16_t>{}(static_cast<int16_t>(e));
  }
};

template <> struct hash<MessageProtocol::InternalMessageType> {
  size_t
  operator()(const MessageProtocol::InternalMessageType &e) const noexcept {
