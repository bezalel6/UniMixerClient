...
Message Message::createAssetRequest(const String& processName, const String& deviceId) {
    Message msg;
    ...
    strncpy(msg.data.asset.processName, processName.c_str(), sizeof(msg.data.asset.processName) - 1);

    return msg;
}

Message Message::createVolumeChange(const String& processName, int volume, const String& deviceId) {
    Message msg;
    msg.type = SET_VOLUME;
    msg.deviceId = deviceId.isEmpty() ? Config::getDeviceId() : deviceId;
    msg.requestId = Config::generateRequestId();
    ...
    strncpy(msg.data.volume.processName, processName.c_str(), sizeof(msg.data.volume.processName) - 1);
    msg.data.volume.volume = volume;

    return msg;
}

Message Message::createAudioStatus(const AudioData& audioData, const String& deviceId) {
    Message msg;
...
Message Message::createAssetResponse(const AssetData& assetData, const String& requestId, const String& deviceId) {
    Message msg;
...
Message Message::fromJson(const String& json) {
    ...
    switch (msg.type) {
        ...
        case ASSET_REQUEST: {
            strncpy(msg.data.asset.processName, doc["processName"] | "", sizeof(msg.data.asset.processName) - 1);
            break;
        }
        ...
        case ASSET_RESPONSE: {
            ...
            strncpy(msg.data.asset.errorMessage, doc["errorMessage"] | "", sizeof(msg.data.asset.errorMessage) - 1);
            strncpy(msg.data.asset.assetDataBase64, doc["assetData"] | "", sizeof(msg.data.asset.assetDataBase64) - 1);
            msg.data.asset.width = doc["width"] | 0;
            msg.data.asset.height = doc["height"] | 0;
            strncpy(msg.data.asset.format, doc["format"] | "", sizeof(msg.data.asset.format) - 1);
            break;
        }

        case SET_VOLUME:
        case VOLUME_CHANGE: {
            strncpy(msg.data.volume.processName, doc["processName"] | "", sizeof(msg.data.volume.processName) - 1);
            msg.data.volume.volume = doc["volume"] | 0;
            strncpy(msg.data.volume.target, doc["target"] | "default", sizeof(msg.data.volume.target) - 1);
            break;
        }
...
const char* Message::typeToString() const {
    switch (type) {
        ...
        case AUDIO_STATUS: return "STATUS_MESSAGE";  // Match existing protocol
        case VOLUME_CHANGE: return "VOLUME_CHANGE";
        case MUTE_TOGGLE: return "MUTE_TOGGLE";
        case ASSET_REQUEST: return "GET_ASSETS";     // Match existing protocol
        case ASSET_RESPONSE: return "ASSET_RESPONSE"; // Match existing protocol
        case GET_STATUS: return "GET_STATUS";        // Match existing protocol
        case SET_VOLUME: return "SET_VOLUME";
        case SET_DEFAULT_DEVICE: return "SET_DEFAULT_DEVICE";
        default: return "UNKNOWN";
    }
}

Message::Type Message::stringToType(const String& str) {
    if (str == "STATUS_MESSAGE") return AUDIO_STATUS;
    if (str == "VOLUME_CHANGE") return VOLUME_CHANGE;
    if (str == "MUTE_TOGGLE") return MUTE_TOGGLE;
    if (str == "GET_ASSETS") return ASSET_REQUEST;
    if (str == "ASSET_RESPONSE") return ASSET_RESPONSE;
    if (str == "GET_STATUS") return GET_STATUS;
    if (str == "SET_VOLUME") return SET_VOLUME;
    if (str == "SET_DEFAULT_DEVICE") return SET_DEFAULT_DEVICE;
    return INVALID;
}
