...
// BinaryProtocol::BinaryProtocolFramer* InterruptMessagingEngine::binaryFramer
// = nullptr;
...
//     // Binary protocol initialized - using working CRC-16-MODBUS algorithm
//     from previous SerialBridge ESP_LOGD(TAG, "Binary protocol framer ready
//     with compatible CRC-16-MODBUS algorithm");
...
//     outgoingMessageQueue = xQueueCreate(MESSAGE_QUEUE_SIZE,
//     sizeof(BinaryMessage*)); core1ProcessingQueue =
//     xQueueCreate(INTERNAL_MSG_QUEUE_SIZE, sizeof(ExternalMessage));
//     core0NotificationQueue = xQueueCreate(INTERNAL_MSG_QUEUE_SIZE,
//     sizeof(InternalMessage));

//     if (!incomingDataQueue || !outgoingMessageQueue || !core1ProcessingQueue
...
//     initialized = true;
//     ESP_LOGI(TAG, "Core 1 Binary Protocol Messaging Engine initialized
...
//     // CRITICAL: Set running = true BEFORE creating task to avoid race
...
//     ESP_LOGI(TAG, "Core 1 Binary Protocol Messaging Engine started
//     successfully"); return true;
// }
...
// void InterruptMessagingEngine::getStats(uint32_t& msgReceived, uint32_t&
// msgSent,
//                                         uint32_t& bufOverruns, uint32_t&
//                                         core1Routed) {
...
// const BinaryProtocol::ProtocolStatistics&
// InterruptMessagingEngine::getBinaryStats() {
//     if (binaryFramer) {
...
//     ESP_LOGI(TAG, "Core 1 Messaging Task started on Core %d",
//     xPortGetCoreID());

//     TickType_t lastWakeTime = xTaskGetTickCount();
//     const TickType_t taskFrequency = pdMS_TO_TICKS(1);  // 1ms cycle time for
...
//         std::vector<String> decodedMessages =
//         binaryFramer->processIncomingBytes(data, length);

//         // Process each decoded JSON message
//         for (const String& jsonMessage : decodedMessages) {
//             ExternalMessage message;
//             if (parseCompleteMessage(jsonMessage.c_str(),
//             jsonMessage.length(), message)) {
//                 messagesReceived++;

//                 // Log to UI with success indicator
//                 LOG_TO_UI(ui_txtAreaDebugLog, ("✓ PARSED: Type=" +
//                 String(static_cast<int>(message.messageType)) +
...
//                 LOG_TO_UI(ui_txtAreaDebugLog, ("PARSE ERROR: " +
//                 jsonMessage).c_str());
//             }
//         }
//     }
...
//             bool success = sendRawData(reinterpret_cast<const
//             char*>(messagePtr->data), messagePtr->length);
...
//     ESP_LOGI(TAG, "Serial initialized successfully at %d baud",
//     MESSAGING_SERIAL_BAUD_RATE); return true;
// }
...
//         try {
//             size_t written = Serial.write(reinterpret_cast<const
//             uint8_t*>(data), length); Serial.flush();  // Ensure all bytes
//             are transmitted
...
//     uxQueueSpacesAvailable(outgoingMessageQueue); bool queueCongested =
//     queueSpacesAvailable < (MESSAGE_QUEUE_SIZE / 4);  // Less than 25% space

//     // Decision matrix:
//     // Small messages + low congestion = Direct transmission (fast)
//     // Large messages OR high congestion = Queue (reliable)

//     bool useDirect = (payloadSize <= DIRECT_TRANSMISSION_THRESHOLD) &&
//     !queueCongested;
...
// bool InterruptMessagingEngine::attemptDirectTransmission(const String&
// payload) {
//     bool success = binaryFramer->transmitMessageDirect(payload, [](uint8_t
//     byte) -> bool {
...
// bool InterruptMessagingEngine::queueMessageForTransmission(const String&
// payload) {
...
//     ESP_LOGI(TAG, "Registered with MessageCore as '%s' transport
//     successfully", Config::TRANSPORT_NAME_SERIAL); return true;
// }
...
//     String status = String("Core1 Engine - Running: ") + (running ? "Yes" :
//     "No") +
//                     ", RX: " + String(messagesReceived) + ", TX: " +
//                     String(messagesSent);

//     if (binaryFramer) {
//         const auto& stats = binaryFramer->getStatistics();
//         status += ", Errors: " + String(stats.crcErrors +
//         stats.framingErrors);
//     }
//     return status;
// }
...
// bool InterruptMessagingEngine::parseCompleteMessage(const char* buffer,
// size_t length, ExternalMessage& message) {
//     if (!buffer || length == 0) {
//         return false;
//     }
...
//         return true;
//     } else {
//         ESP_LOGW(TAG, "Failed to parse message: %s",
//         parseResult.getError().c_str()); return false;
//     }
// }

// void InterruptMessagingEngine::routeExternalMessage(const ExternalMessage&
// message) {
...
//         // Use MessageCore's routing logwc by converting to internal message
//         first std::vector<InternalMessage> internalMessages =
...
// void InterruptMessagingEngine::processExternalMessageOnCore1(const
// ExternalMessage& message) {
//     if (messageCore) {
...
// void InterruptMessagingEngine::routeInternalMessage(const InternalMessage&
// message) {
//     if (xQueueSend(core0NotificationQueue, &message, 0) != pdTRUE) {
//         ESP_LOGD(TAG, "Failed to route internal message to Core 0 - queue
//         full");
//     }
// }
