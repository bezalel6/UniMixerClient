...
// CRC16 implementation simplified - using direct algorithm from working
// SerialBridge

uint16_t CRC16Calculator::calculate(const uint8_t *data, size_t length) {
  // Use the working CRC-16-MODBUS algorithm from previous SerialBridge
  // implementation
  uint16_t crc = 0xFFFF;
...
uint16_t CRC16Calculator::calculate(const String &data) {
  return calculate(reinterpret_cast<const uint8_t *>(data.c_str()),
                   data.length());
}
...
bool validateFrame(const uint8_t *frame, size_t frameLength) {
  ...
  if (frame[0] != MSG_START_MARKER ||
      frame[frameLength - 1] != MSG_END_MARKER) {
    return false;
  }
...
BinaryProtocolFramer::BinaryProtocolFramer()
    : currentState_(ReceiveState::WaitingForStart), headerBufferSize_(0),
      payloadBufferSize_(0), expectedPayloadLength_(0), expectedCrc_(0),
      messageType_(0), messageStartTime_(0), isEscapeNext_(false) {
...
BinaryProtocolFramer::encodeMessage(const String &jsonPayload) {
  ...
  // Length (4 bytes, little-endian) - length of ORIGINAL payload before
  // escaping
  ...
  for (size_t i = 0; i < payloadLength; i++) {
    ...
    if (byte == MSG_START_MARKER || byte == MSG_END_MARKER ||
      ...
      frame.push_back(
          byte ^
          MSG_ESCAPE_XOR); // XOR with 0x20 for escaping (like SerialBridge)
    } else {
  ...
  ESP_LOGD(
      TAG,
      "Encoded message: %zu bytes payload -> %zu bytes frame (CRC: 0x%04X)",
      payloadLength, frame.size(), crc);

  return frame;
}

bool BinaryProtocolFramer::encodeMessage(const String &jsonPayload,
...
bool BinaryProtocolFramer::transmitMessageDirect(
    const String &jsonPayload, std::function<bool(uint8_t)> writeByteFunc) {
  if (jsonPayload.isEmpty()) {
  ...
  if (payloadLength > MAX_PAYLOAD_SIZE) {
    ESP_LOGE(TAG, "Payload exceeds maximum size for direct transmission: %zu",
             payloadLength);
    return false;
  }
...
std::vector<String>
BinaryProtocolFramer::processIncomingBytes(const uint8_t *data, size_t length) {
...
std::vector<String>
BinaryProtocolFramer::processIncomingBytes(const std::vector<uint8_t> &data) {
  return processIncomingBytes(data.data(), data.size());
}

std::vector<uint8_t>
BinaryProtocolFramer::applyEscapeSequences(const uint8_t *data, size_t length) {
...
void updateCRCAlgorithm(uint16_t polynomial, uint16_t initial, bool reflect) {
  ESP_LOGI("BinaryProtocol",
           "Updating CRC algorithm: Poly=0x%04X, Init=0x%04X, Reflect=%s",
