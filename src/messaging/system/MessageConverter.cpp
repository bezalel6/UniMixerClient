...
InternalMessage
MessageConverter::createAudioVolumeMessage(const String &processName,
                                           int volume) {
  ...
  ESP_LOGD(TAG, "Created audio volume message: %s = %d", processName.c_str(),
           volume);
  return msg;
}

InternalMessage MessageConverter::createUIUpdateMessage(const String &component,
                                                        const String &data) {
  InternalMessage msg(MessageProtocol::InternalMessageType::UI_UPDATE);
...
InternalMessage
MessageConverter::createSystemStatusMessage(const String &status) {
...
InternalMessage MessageConverter::createWifiStatusMessage(const String &status,
                                                          bool connected) {
  ...
  ESP_LOGD(TAG, "Created WiFi status message: %s (connected: %s)",
           status.c_str(), connected ? "yes" : "no");
  return msg;
}

InternalMessage MessageConverter::createNetworkInfoMessage(const String &ssid,
                                                           const String &ip) {
  ...
  ESP_LOGD(TAG, "Created network info message: %s (%s)", ssid.c_str(),
           ip.c_str());
  return msg;
}

InternalMessage MessageConverter::createSDStatusMessage(const String &status,
                                                        bool mounted) {
  ...
  ESP_LOGD(TAG, "Created SD status message: %s (mounted: %s)", status.c_str(),
           mounted ? "yes" : "no");
  return msg;
}

InternalMessage
MessageConverter::createAudioDeviceChangeMessage(const String &deviceName) {
  InternalMessage msg(
      MessageProtocol::InternalMessageType::AUDIO_DEVICE_CHANGE);
...
InternalMessage MessageConverter::createCoreToCoreSyncMessage(uint8_t fromCore,
                                                              uint8_t toCore) {
  MessageProtocol::InternalMessageType msgType =
      (toCore == 1) ? MessageProtocol::InternalMessageType::CORE0_TO_CORE1
                    : MessageProtocol::InternalMessageType::CORE1_TO_CORE0;
  ...
  ESP_LOGD(TAG, "Created core sync message: Core %d -> Core %d", fromCore,
           toCore);
  return msg;
}

InternalMessage
MessageConverter::createDebugUILogMessage(const String &logMessage) {
