...
MessageParser::parseExternalMessage(const String &jsonPayload) {
  ...
  ESP_LOGW(TAG, "Got payload: %s", jsonPayload.c_str());
  JsonObject obj = doc.as<JsonObject>();
...
MessageParser::parseAudioStatusData(const ExternalMessage &message) {
  ...
  if (message.messageType !=
    ...
    return ParseResult<AudioStatusData>::createError(
        "Invalid message type for audio status");
  }
  ...
  try {
    ...
    if (message.parsedData[DEFAULT_DEVICE].is<JsonObject>()) {
      ...
      data.defaultDevice.friendlyName =
          message.parsedData[DEFAULT_DEVICE][FRIENDLY_NAME].as<String>();
      ...
      data.defaultDevice.dataFlow =
          message.parsedData[DEFAULT_DEVICE][DATA_FLOW].as<String>();
      data.defaultDevice.deviceRole =
          message.parsedData[DEFAULT_DEVICE][DEVICE_ROLE].as<String>();
    ...
    data.originatingDeviceId =
        message.parsedData[ORIGINATING_DEVICE_ID].as<String>();
    data.originatingRequestId =
        message.parsedData[ORIGINATING_REQUEST_ID].as<String>();
...
MessageParser::parseAssetResponseData(const ExternalMessage &message) {
  ...
  if (message.messageType !=
    ...
    return ParseResult<AssetResponseData>::createError(
        "Invalid message type for asset response");
  }
