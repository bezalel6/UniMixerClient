...
void AudioUI::onVolumeSliderChanged(int volume) {
  ...
  ESP_LOGI(TAG,
           "Volume slider changed to: %d - applying with reactive feedback",
           volume);
  ...
  if (AudioManager::getInstance().getCurrentTab() ==
      Events::UI::TabState::MASTER) {
...
void AudioUI::onDeviceDropdownChanged(lv_obj_t *dropdown,
...
void AudioUI::onTabChanged(Events::UI::TabState newTab) {
  ...
  ESP_LOGI(TAG, "Tab changed to: %s - triggering reactive updates",
           AudioManager::getInstance().getTabName(newTab));

  // Set the new tab first
  AudioManager::getInstance().setCurrentTab(newTab);

  // Trigger immediate smart auto-selection for the new tab
  // This happens after the tab is set so auto-selection logic can work
  // correctly
...
void AudioUI::onAudioStateChanged(const AudioStateChangeEvent &event) {
  ESP_LOGI(
      TAG,
      "Handling audio state change event: %d - triggering reactive UI updates",
      (int)event.type);
...
void AudioUI::updateTabVisibility() {
  // This could be expanded to show/hide different UI elements based on the
  // current tab For now, the tab-specific UI is handled by the LVGL tab view
  // itself
...
void AudioUI::setDropdownSelection(lv_obj_t *dropdown,
                                   const String &deviceName) {
  if (!dropdown) {
    return;
  }
...
int AudioUI::findDeviceIndexInDropdown(lv_obj_t *dropdown,
...
void AudioUI::updateSingleTabLogo() {
  ...
  // Get logo path from simplified LogoStorage

  if (Logo::LogoStorage::getInstance().hasLogo(processName)) {
    String logoPath = Logo::LogoStorage::getInstance().getLogoPath(processName);
