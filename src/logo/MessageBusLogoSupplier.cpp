...
//           false, processName, request.requestId.c_str(), "Insufficient
//           memory");
//       callback(response);
//     }
//     return false;
...
//   // If no requests are currently active and sufficient memory, send
//   immediately if (pendingRequests.size() <
...
//   if (shouldSave) {
//     // CRITICAL: Defer logo saving to prevent stack overflow on messaging
//     task ESP_LOGI(TAG, "Deferring logo save for: %s (%zu bytes)",
...
//       AssetResponse response = createAssetResponse(
//           false, nextRequest.processName.c_str(),
...
// bool MessageBusLogoSupplier::saveAssetToStorage(const AssetResponse
// &response) {
...
//   ESP_LOGI(TAG, "Saving PNG logo for process: %s (%zu bytes)",
//            response.processName.c_str(), response.assetDataSize);

//   // Use the new LogoManager to save the PNG data
//   // The response.assetData is PNG format from the server
//   String logoPath = Logo::LogoManager::getInstance().saveLogo(
//       response.processName.c_str(), response.assetData,
//       response.assetDataSize, Logo::LogoStorage::FileType::PNG);

//   if (!logoPath.isEmpty()) {
//     ESP_LOGI(TAG, "Successfully saved PNG logo for: %s at path: %s",
//              response.processName.c_str(), logoPath.c_str());
//     return true;
//   } else {
//     ESP_LOGE(TAG, "Failed to save PNG logo for: %s",
//              response.processName.c_str());
//     return false;
//   }
// }
