...
void LogoSupplierManager::deinit() {
  ...
  if (supplierMutex &&
      xSemaphoreTake(supplierMutex, pdMS_TO_TICKS(5000)) == pdTRUE) {
...
bool LogoSupplierManager::registerSupplier(LogoSupplier *supplier,
                                           int priority) {
  ...
  for (const auto &entry : suppliers) {
    if (entry.supplier == supplier) {
      ESP_LOGW(TAG, "Supplier already registered: %s",
  ...
  if (!supplier->init()) {
    ESP_LOGE(TAG, "Failed to initialize supplier: %s",
             supplier->getSupplierType());
    xSemaphoreGive(supplierMutex);
    return false;
  }
  ...
  ESP_LOGI(TAG, "Registered supplier: %s (priority: %d)",
           supplier->getSupplierType(), priority);
...
bool LogoSupplierManager::requestLogo(const char *processName,
  ...
  ESP_LOGW(TAG, "No available suppliers to handle logo request for: %s",
           processName);
  xSemaphoreGive(supplierMutex);
  return false;
}
...
String LogoSupplierManager::getStatus() const {
  ...
  if (initialized && supplierMutex &&
      xSemaphoreTake(supplierMutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
    for (size_t i = 0; i < suppliers.size(); i++) {
      const auto &entry = suppliers[i];
      status += "- Supplier " + String(i + 1) + ": " +
                String(entry.supplier->getSupplierType());
      status += " (priority: " + String(entry.priority) +
                ", enabled: " + String(entry.enabled ? "Yes" : "No");
