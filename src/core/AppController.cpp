...
bool init(void) {
    ...
    // CRITICAL: Start LVGL task EARLY so BSOD can work if components fail
    BOOT_STATUS("Starting LVGL task...");
    BootProgress::updateProgress(20);
    INIT_CRITICAL(Application::TaskManager::init(),
                  "Failed to start LVGL task. System cannot handle display updates.");
    ...
    // TaskManager already initialized earlier - no need to start again
